...
public class SumBiFunctionTest {
	...
	public void apply_ConstantNodes_ReturnSumValue() throws Exception {
		...
		ComputeTree.Node sumNode = ComputeTree.Node.builder()
				...
				.build();
		sumNode.addHighers(constantNode1, constantNode2);

		ComputeTree computeTree = ComputeTreeUtils.create(constantNode1, constantNode2, sumNode);
		...
		// Assert
		Value value = valueTree.getIdNodes().get(sumNode.getId()).getValue();
