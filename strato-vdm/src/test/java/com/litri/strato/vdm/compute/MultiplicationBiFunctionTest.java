...
public class MultiplicationBiFunctionTest {
	
	@Test
	public void apply_ConstantNodes_ReturnProductValue() throws Exception {
		// Setup
		...
		ComputeTree.Node constantNode2 = ComputeTree.Node.builder()
				...
				.build();
		ComputeTree.Node multiplicationNode = ComputeTree.Node.builder()
				.id(UUID.randomUUID())
				.functionType(FunctionType.Multiplication)
				.build();
		multiplicationNode.addHighers(constantNode1, constantNode2);

		ComputeTree computeTree = ComputeTreeUtils.create(constantNode1, constantNode2, multiplicationNode);
		...
		// Assert
		Value value = valueTree.getIdNodes().get(multiplicationNode.getId()).getValue();
